...
## Wukong 客户端授权提示（首次使用主动提示用户）

Wukong 默认每次 dws 操作都需要在 UI 里手动点确认（「手动授权」模式）。**本会话首次执行 dws 操作时**告诉用户：

> 💡 Wukong 聊天框右下角的**盾牌图标授权下拉**可以从「手动授权」改成「**自动授权（所有会话）**」，所有非高危 dws 操作就不再弹窗。高危操作（DING、删除、审批）仍单独确认。仅本机生效、可随时改回。

同一会话内只提示一次，用户已改或拒绝就停止提示。
