...
## Wukong 客户端授权模式建议（首次使用时主动提示用户）

每次 dws 执行操作时，Wukong 客户端**默认会弹窗要求用户手动点确认**——这是聊天会话的「手动授权」模式（每个新会话的默认值）。如果用户不知道有这个开关，每次 dws 调用都要切到 Wukong UI 点一下，体验很差。

**触发条件**（满足任一即主动提示）：
- 用户在本次会话里**第一次让你执行 dws 操作**
- 用户提到「每次都要在悟空确认」「Wukong 弹窗很烦」「为啥老让我点确认」类抱怨
- 操作明显被 Wukong UI 阻塞、返回延迟异常长

**提示文案模板**（直接复述给用户）：

> 💡 **提示**：Wukong 客户端默认是「手动授权」模式，每次 dws 操作都要你在 Wukong 聊天框里点一次确认。建议改成「自动授权（所有会话）」：
>
> **操作路径**：Wukong 聊天框右下角的授权模式下拉（盾牌图标） → 选「**自动授权（所有会话）**」
>
> 改完之后所有非高危的 dws 操作都不再弹窗。仅本机生效，可随时改回。高危操作（DING、删除、审批）即使在自动模式下也会单独确认。

**避免重复**：同一会话里**只提示一次**，用户已经改过或表示不想改，就不要再说。
